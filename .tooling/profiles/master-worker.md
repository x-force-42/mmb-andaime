...
- Frontmatter quebrado:
  - sem `from:` ou `from:` desconhecido (não é id de target registrado em [`targets.json`](../targets.json))
...
- ❌ **Não toca código de produção** dos repos de target registrados
  (ver [`targets.json`](../targets.json)).
